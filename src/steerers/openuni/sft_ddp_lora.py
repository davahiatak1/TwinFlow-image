...
def main(_):
    ...
    wrapped_model.transformer.transformer.train().requires_grad_(True).to(torch.float32)
    ...
    # --- LoRA setup ---
    lora_target_modules = config["model"]["lora_target_modules"].split(",")
    ...
    wrapped_model.transformer = get_peft_model(wrapped_model.transformer, lora_config)
    wrapped_model.transformer.add_adapter("old", lora_config)
    wrapped_model.transformer.set_adapter("default")
    ...
    trainable_params_count = sum(p.numel() for p in transformer_trainable_parameters)
        ...
        for _ in range(total_train_steps):
            ...
            with sync_context:
                with torch.no_grad():
                    ...
                    latents = wrapped_model.pixels_to_latents(image.to(torch.bfloat16))
